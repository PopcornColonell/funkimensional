// im not even using this class but i might as well
...
class FlxTypedEmitter<T:FlxSprite & IFlxParticle> extends FlxTypedGroup<T> {
	...
	*/
	public function loadParticles(Graphics:FlxGraphicAsset, Quantity:Int = 50, bakedRotationAngles:Int = 16, Multiple:Bool = false, AutoBuffer:Bool = false):FlxTypedEmitter<T> {
		...
		for (i in 0...Quantity) add(loadParticle(Graphics, Quantity, bakedRotationAngles, Multiple, AutoBuffer, totalFrames));
		return this;
	}
	...
	*/
	...
	public function emitParticle():T {
		...
		} else particle.velocityRange.active = false;
		...
		// Restart animation
		if (particle.animation.curAnim != null) particle.animation.curAnim.play(true, particle.animation.curAnim.reversed);
		particle.onEmit();
		return particle;
	}
	...
	*/
	...
	inline function get_solid():Bool return allowCollisions.has(ANY);

	function set_solid(Solid:Bool):Bool {
		if (Solid) allowCollisions = ANY;
		else allowCollisions = NONE;
		return Solid;
	}
}
