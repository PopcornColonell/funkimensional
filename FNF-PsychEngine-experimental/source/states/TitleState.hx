...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		tikyDance = new FlxSprite(0, 0);
		tikyDance.frames = Paths.getSparrowAtlas('mainmenu/tricky/happymadnessday');
		tikyDance.animation.addByPrefix('idle', "Símbolo 2");
		tikyDance.scale.x = 0.4;
		tikyDance.scale.y = 0.4;
		tikyDance.screenCenter();
		tikyDance.y -= 75;
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized) {
			tikyDance.animation.play('idle');
			FlxTween.tween(tikyDance.scale, {x: 0.395, y: 0.395}, 0.25, {ease: FlxEase.quadIn, loopDelay: 0,
				onComplete: function(twn:FlxTween) {
					FlxTween.tween(tikyDance.scale, {x: 0.4, y: 0.4}, 0.25, {ease: FlxEase.quadIn, loopDelay: 0});
				}
			});
		}
