...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'nevada': new states.stages.Nevada(); //Week T BABY
		}
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					case 0:
						if ((curSong == 'hellish-chuckle')) {
							var buffervid:VideoHandler = new VideoHandler();
							var bufferfilepath:String = Paths.video('cutesenahellclown');
							buffervid.play(bufferfilepath);
							buffervid.dispose();
						}
	...
	override function beatHit()
	{
		...
		if ((curSong == 'hellish-chuckle') && (curBeat == 1)) {
			GameOverSubstate.hankdead2 = false;
			PauseSubState.tikymad = false;
			PauseSubState.hellmad = false;
		}
		if ((curSong == 'hellish-chuckle') && (curBeat == 336)) {
			PauseSubState.tikymad = true;
		}
		if ((curSong == 'hellish-chuckle') && (curBeat == 546)) {
			startVideo('cutesenahellclown');
		}
		if ((curSong == 'hellish-chuckle') && (curBeat == 574)) {
			inCutscene = false;
			camZooming = false;
		}
		if ((curSong == 'hellish-chuckle') && (curBeat == 580)) {
			inCutscene = false;		
			GameOverSubstate.hankdead2 = true;
			PauseSubState.tikymad = false;
			PauseSubState.hellmad = true;
		}
		if ((curSong == 'hellish-chuckle') && (curBeat == 588)) {
			camZooming = true;	
		}
	}
